...
class SignInController extends \yii\web\Controller
{
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => [
                            'signup','access', 'login', 'request-password-reset', 'reset-password', 'oauth', 'activation'
                        ],
                        'allow' => true,
                        'roles' => ['?']
                    ],
    ...
    public function actionLogin()
    {
        ...
        if ($model->load(Yii::$app->request->post(),'') && $model->login()) {
            ...
            return array_merge(Yii::$app->params['return_success'],['msg'=>'登录成功','data'=>['token' => $accessToken]]);
        } else {
            return array_merge(Yii::$app->params['return_error'],['msg'=>'登录失败']);
        }
    }
    ...
    public function actionAccess($token)
    {
        ...
        if ($accessToken) {
            if (User::findIdentityByAccessToken($accessToken)) {
                return array_merge(Yii::$app->params['return_success'],['msg'=>'登录成功']);
            }
        }
        return array_merge(Yii::$app->params['return_error'],['msg'=>'登录失败']);
    }
